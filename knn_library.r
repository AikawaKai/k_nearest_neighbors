...
parallelExternalCrossValidationWithInnerOptimization <- function(data, myK, k_fold, pca, num_feat){
  ...
  print("Std Precision class1: ")
  print(sd(as.numeric(precision1)))
  print("Precision class2: ")
  print(mean(as.numeric(precision2)))
  print("Std Precision class2: ")
  print(sd(as.numeric(precision2)))
  cat("\n")
  print("Std accuracy: ")
  print(sd(as.numeric(final_accuracy)))
...
parallelExternalCrossValidationWithInnerOptimizationSeq <- function(data, myK, k_fold, pca, num_feat){
  ...
  final_res <- parLapply(cl, 1:k_fold, function(x) c(myKnnWithCppSeq(trainings[[x]], tests[[x]], res_k[[x]])))
  accuracy_vals <-lapply(final_res, getAccuracyFromCM)
  precision1 <-lapply(final_res, function (x) c(getPrecisionFromCM(x, 1)))
  precision2 <-lapply(final_res, function (x) c(getPrecisionFromCM(x, 2)))
  stopCluster(cl)
  print("Precision class1: ")
  ...
  cat("\n")
  print("Std accuracy: ")
  print(sd(as.numeric(accuracy_vals)))
...
selectBestKByCrossValidationSeq <- function(data, myK, k_fold, pca, num_feat){
  ...
  clusterExport(cl, list("path", "myK", "getTestErrorFromAccuracy", "plotMyResults", 
                         "getPrecisionFromCM", "innerCrossValidationSeq",
  ...
  res_k <- list(length(myK))
  for(i in 1:length(myK)){
    res_k_i <- parLapply(cl, 1:k_fold, function(x) c(myKnnWithCppSeq(trainings[[x]], tests[[x]], myK[[i]])))
    res_k_i <- lapply(res_k_i, getAccuracyFromCM)
    res_k[[i]] <- mean(as.numeric(res_k_i))
  }
  stopCluster(cl)
